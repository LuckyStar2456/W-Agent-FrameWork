...
- Dedicated OpenAI Responses, vLLM differences, reasoning deltas, token-pass-through execution, registration probes, and CLI/TUI configuration remain `Planned`. The separate `ModelExecutor` provides collecting retry/failover.

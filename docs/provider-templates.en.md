...
- OpenAI Responses and vLLM-specific differences are implemented and can be assembled by strict local configuration and the CLI/TUI run and probe paths. Reasoning-content deltas, OpenAI hosted-tool events, multimodal output, and cross-stream recovery remain `Planned`.

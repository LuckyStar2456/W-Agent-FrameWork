...
The current repository and latest alpha version is `2.0.0a2`: the Phase 1 microkernel, Phase 2 model foundation, Phase 3 agent/tool foundation, and Phase 4 local-workflow foundation are implemented, while 1.x APIs remain available. Every new capability updates both Chinese and English documentation and receives a status label.

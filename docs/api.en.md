...
| Group | API |
|---|---|
| Agent | `BaseAgent`, `LegacyAgentAdapter`, `AgentDefinition`, `AgentLoop`, `ReactAgentLoop`, `RunContext`, `RunEvent`, `RunResult`, `RunStore`, `RunCheckpointSummary`, `JsonlRunStore`, `TokenBudget`, `TokenEstimator`, `TokenEstimate`, `CharacterTokenEstimator`, `CostBudget`, `CostEstimator`, `CostEstimateRequest`, `CostEstimateRoute`, `CostEstimate`, `PricingCostEstimator` |
| Session | `SessionManager`, `RunEventCallback`, `SessionRecord`, `SessionRunRecord`, `AgentUsageSummary`, `InMemorySessionStore`, `JsonSessionStore` |
...
`ReactAgentLoop` is an ordinary implementation built only on public model/tool contracts and can be replaced as a whole through the same protocol. It runs a bounded model/tool cycle, appends through `RunStore` before event visibility, and returns `pending_tool_call` plus `checkpoint_id` when approval is required. `RunStore.list_checkpoints()` exposes `RunCheckpointSummary` without prompts, argument values, or outputs. `AgentDefinition.emit_text_deltas=True` explicitly switches to the safe model stream and produces persisted `MODEL_TEXT_DELTA` events; collecting mode remains the default, and the TUI never renders delta bodies. `TokenBudget` provides cumulative token limits; construction may inject a `TokenEstimator` for pre-call protection and visibility through `TOKEN_ESTIMATED`, `TOKEN_ESTIMATE_UNAVAILABLE`, and `TOKEN_BUDGET_WARNING`. `CharacterTokenEstimator` is an explicit opt-in heuristic template that rejects image/audio input and never claims exact tokenizer behavior. `CostBudget` combines with a replaceable `PricingResolver` and application-supplied versioned `PriceTable` for cost metering and hard stops. Explicit pre-call protection may inject a `CostEstimator`; the default `PricingCostEstimator` derives `COST_ESTIMATED`, `COST_ESTIMATE_UNAVAILABLE`, and `COST_BUDGET_WARNING` from a prompt-free route/token/replay envelope. `TOKEN_USAGE`/`COST_USAGE`, `RunResult`, and checkpoints expose and preserve token values, costs, attempt ledgers, and completeness. `SessionManager.run_agent()`/`resume_agent()` accept an optional synchronous or asynchronous `RunEventCallback` and project live events in public stream order; the built-in ReAct loop persists each event before exposing it. `SessionManager.agent_usage()` returns prompt/output-free `AgentUsageSummary` values with cross-session token, cost, and completeness totals by agent, excluding archived sessions by default. `customer_support_agent()` and `coding_agent()` build fully overridable ordinary definitions and bind no model, tool, or authority. General multimodal/tool event replay remains `Planned`. See [Agent runtime](./agents.en.md) and [Session lifecycle](./sessions.en.md).

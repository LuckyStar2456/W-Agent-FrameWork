...
当前包元数据和最新 Alpha 均为 `2.0.0a3`；main 已继续承载未发布改动。Phase 1 微内核、Phase 2 模型基础、Phase 3 Agent/工具基础和 Phase 4 本地 Workflow 基础已经实现，1.x API 继续保留。任何新能力都必须同步更新中英文文档，并标记发布边界与状态。
...
- 拥有 Provider/传输的装配必须公开幂等异步关闭；CLI/TUI 短生命周期入口在同一事件循环内完成运行与关闭。
...
模型测试优先使用 `ScriptedModelProvider` 等确定性 Mock；真实 API 测试必须显式启用并避免泄漏凭据。模型录制/回放必须显式授权敏感内容并保护 Cassette 文件，评测报告默认不得持久化 Prompt、输出和异常正文。详见[本地测试、模型回放与评测](./testing-evaluation.md)。

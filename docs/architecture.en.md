...
Status: the Phase 2A protocols, registry, routing, and probe framework plus the Phase 2B OpenAI-compatible provider, generic HTTP mapping layer, initial vendor templates, and collecting invocation/failover executor are `Implemented`; OpenAI Responses/vLLM differences and pass-through streaming execution are `Planned`.

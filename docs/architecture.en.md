...
The current `Experimental` local evaluation layer provides network-free scripted models, explicitly authorized JSONL model recording/sequential replay, replaceable scorers, and token-completeness, latency, error, and tool-success metrics. Recording is an ordinary replaceable model-provider decorator, and evaluation targets depend only on public `RunResult` values, so neither has kernel privilege. Built-in customer-support/coding suites, pricing/cost metrics, and CLI/TUI evaluation screens remain `Planned`; an online evaluation platform is outside project scope. See [Local testing, model replay, and evaluation](./testing-evaluation.en.md).

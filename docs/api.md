...
`ReactAgentLoop` 是只使用公开模型与工具协议的普通实现，可以被同协议 Loop 整体替换。它执行有界模型/工具循环，通过 `RunStore` 在事件可见前追加记录，并在需要审批时返回 `pending_tool_call` 与 `checkpoint_id`。`RunStore.list_checkpoints()` 提供不含 Prompt/参数值/输出的 `RunCheckpointSummary`。`AgentDefinition.emit_text_deltas=True` 显式切换到安全模型流并产生持久化的 `MODEL_TEXT_DELTA`；默认仍为收集模式，TUI 不显示增量正文。`TokenBudget` 提供 Run 级累计 Token 限制；构造 Loop 时可注入 `TokenEstimator`，通过 `TOKEN_ESTIMATED`、`TOKEN_ESTIMATE_UNAVAILABLE` 和 `TOKEN_BUDGET_WARNING` 提供调用前保护与软阈值可见性。`CharacterTokenEstimator` 是必须显式启用且不处理图片/音频的启发式模板，不会冒充精确分词器。`CostBudget` 配合可替换 `PricingResolver` 和应用提供的版本化 `PriceTable` 提供费用计量与硬停止；显式调用前保护可注入 `CostEstimator`，默认 `PricingCostEstimator` 从无 Prompt 的路由/Token/重放包络产生 `COST_ESTIMATED`、`COST_ESTIMATE_UNAVAILABLE` 与 `COST_BUDGET_WARNING`。`TOKEN_USAGE`/`COST_USAGE`、`RunResult` 和 Checkpoint 公开并保留 Token、费用、尝试账本与完整性。`SessionManager.run_agent()`/`resume_agent()` 接受可选同步或异步 `RunEventCallback`，按公开流顺序把实时事件投影给应用；内置 ReAct Loop 会先持久化再暴露事件。`SessionManager.agent_usage()` 返回不含 Prompt/输出的 `AgentUsageSummary`，按 Agent 汇总多个 Session 的 Token、费用与完整性，默认排除归档 Session。`customer_support_agent()` 和 `coding_agent()` 只构建可完全覆盖的普通 Definition，不绑定模型、工具或权限。多模态/工具事件通用回放仍为 `Planned`。详见[Agent Runtime](./agents.md)与[Session](./sessions.md)。
...
`ToolRegistry` 在统一注册表中按版本和 Scope 注册 `ToolBinding`。`ToolExecutor.execute()` 依次解析、校验、执行策略、处理超时/取消并写入审计。默认 `PermissionPolicy` 检查本地授予的权限，`SideEffectApprovalPolicy` 对写入、破坏性和外部副作用要求调用 ID 逐次批准。工具 Definition 不持有执行策略，默认执行器不重试。`python_tool()`、`http_tool()`、`command_tool()`、`mcp_tool()`、`sandbox_command_tool()`、`McpClient`、`McpStdioTransport`、`McpStreamableHttpTransport` 和 `discover_mcp_bindings()` 已实现；旧版 MCP 初始化协商、MRTR 自动输入交换和订阅流仍为 `Planned`。详见[工具注册、策略与执行](./tools.md)。
